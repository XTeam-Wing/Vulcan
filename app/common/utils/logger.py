...
import os
import inspect
from app.config.settings import LOGGER_PATH
...
def _format_message(level, message):
    ...
    message = "{time} {level} - {module}[{line}]: {message}".format(
        ...
        module=module_name, line=line_no, level=level, message=str(message),
    )
    return message
...
class Logger:
    def __init__(self):
        try:
            # 从上下文中获取配置文件中的日志路径配置
            self._log_file = LOGGER_PATH + '/' + "Vulcan-{}.log".format(
                time.strftime("%Y_%m_%d", time.localtime())
            )
            if not os.path.exists(LOGGER_PATH):
                os.makedirs(LOGGER_PATH)
            self._logger = logging.getLogger("icewing")
    ...
    def warning(self, message):
        self._logger.warning(_format_message("WARN", str(message)))
